...
author: "Eugene Brusilovskiy, edited by Hang Zhao, Jiahang Li, Ling Chen"
date: "`r Sys.Date()`"
...
knitr::opts_knit$set(root.dir = "/Users/hangzhao/Documents/R/MUSA500 Assignment 2")
```
...
```{r warning=FALSE, message=FALSE, cache=FALSE}
shp <-st_read(dsn ="/Users/hangzhao/Documents/R/Lecture 1 - RegressionData.shp/RegressionData.shp")
```
...
hist(shp$PCTBACHMOR, breaks = 50)
hist(shp$PCTVACANT, breaks = 50)
hist(shp$PCTSINGLES, breaks = 50)
hist(shp$NBelPov100, breaks = 50)
```
...
shp$LNPCTBACHMOR <- log(shp$PCTBACHMOR + 1)
shp$LNPCTSINGLES <- log(shp$PCTSINGLES + 1)

par(oma=c(0,0,1,0)) 
par(mfrow=c(2,3)) 
hist(shp$LNPCTSINGLES, breaks = 50)
hist(shp$LNMEDHVAL, breaks = 50)
hist(shp$LNPCTVACANT, breaks = 50)
hist(shp$LNNBELPOV, breaks = 50)
hist(shp$LNPCTBACHMOR, breaks = 50)
```
...
LISA<-localmoran(shp$LNMEDHVAL, queenlist)
head(LISA)
...
hl.plot<-function(df, listw){
  ...
  par(mar=c(0,0,0,0))
  plot<-plot(shp$geometry,border="gray90",lwd=0.5,col=colors[findInterval(quadrant,brks,all.inside=FALSE)])
}
...
```{r warning=FALSE, message=FALSE, cache=FALSE}
reg<-lm(formula=LNMEDHVAL ~ PCTSINGLES + PCTVACANT + PCTBACHMOR + LNNBELPOV, data=shp)
summary(reg)
...
```{r warning=FALSE, message=FALSE, cache=FALSE}
lagreg<-lagsarlm(formula=LNMEDHVAL ~ PCTSINGLES + PCTVACANT + PCTBACHMOR + LNNBELPOV, data=shp, queenlist)
summary(lagreg)
...
```{r warning=FALSE, message=FALSE, cache=FALSE}
shp$reslag <- reslag
slag.Residuals.Map<-tm_shape(shp)+
  tm_fill(col='reslag', style='quantile', title='Spatial Lag Residuals', 
          palette ='Blues')+
  tm_layout(frame=FALSE, title = 'Spatial Lag Residuals')
slag.Residuals.Map
```
...
```{r warning=FALSE, message=FALSE, cache=FALSE}
errreg<-errorsarlm(formula=LNMEDHVAL ~ PCTSINGLES + PCTVACANT + PCTBACHMOR + LNNBELPOV, data=shp, queenlist)
...
```{r warning=FALSE, message=FALSE, cache=FALSE}
shp$reserr <- reserr
serror.Residuals.Map<-tm_shape(shp)+
  tm_fill(col='reserr', style='quantile', title='Spatial Error Residuals', 
          palette ='Blues')+
  tm_layout(frame=FALSE, title = 'Spatial Error Residuals')
serror.Residuals.Map
```
...
class (shps)
bw<-gwr.sel(formula=LNMEDHVAL ~ PCTSINGLES + PCTVACANT + PCTBACHMOR + LNNBELPOV, 
            data=shps,
            method = "aic",
            adapt = TRUE)
bw
...
bw_fixed<-gwr.sel(formula=LNMEDHVAL ~ PCTSINGLES + PCTVACANT + PCTBACHMOR + LNNBELPOV, 
            data=shps,
            method = "aic",
            adapt = FALSE)
bw_fixed
...
```{r warning=FALSE, message=FALSE, cache=FALSE}
gwrmodel<-gwr(formula=LNMEDHVAL ~ PCTSINGLES + PCTVACANT + PCTBACHMOR + LNNBELPOV,
              data=shps,
...
```{r warning=FALSE, message=FALSE, cache=FALSE}
gwrmodel_fixed<-gwr(formula=LNMEDHVAL ~ PCTSINGLES + PCTVACANT + PCTBACHMOR + LNNBELPOV,
              data=shps,
...
coefPCTSINGLES<-tm_shape(shps)+
  tm_fill(col='coefPCTSINGLESst', breaks=c(-Inf, -6, -4, -2, 0, 2, 4, 6, Inf), title='Standardized coefficient of PCTSINGLES', 
          palette ='-RdBu')+
  tm_layout(frame=FALSE, title = 'Percentage of Single Household', title.size = 0.8, legend.title.size = 0.8)
...
coefPCTVACANT<-tm_shape(shps)+
  tm_fill(col='coefPCTVACANTst', breaks=c(-Inf, -6, -4, -2, 0, 2, 4, 6, Inf), title='Standardized coefficient of PCTVACANT', 
          palette='-RdBu')+
  tm_layout(frame=FALSE, title = 'Percentage of Housing Vacant', title.size = 0.8, legend.title.size = 0.8)
...
coefPCTBACHMOR<-tm_shape(shps)+
  tm_fill(col='coefPCTBACHMORst', breaks=c(-Inf, -6, -4, -2, 0, 2, 4, 6, Inf), title='Standardized coefficient of PCTBACHMOR', 
          palette='-RdBu')+
  tm_layout(frame=FALSE, title = 'Percentage of Bachelor Degree', title.size = 0.8, legend.title.size = 0.8)
...
coefLNNBELPOV<-tm_shape(shps)+
  tm_fill(col='coefLNNBELPOVst', breaks=c(-Inf, -6, -4, -2, 0, 2, 4, 6, Inf), title='Standardized coefficient of LNNBELPOV', 
          palette='-RdBu')+
  tm_layout(frame=FALSE, title = 'Number of Households Living in Poverty', title.size = 0.8, legend.title.size = 0.8)

tmap_arrange(coefPCTSINGLES, coefPCTVACANT, coefPCTBACHMOR, coefLNNBELPOV, ncol=2, nrow=2)
```
